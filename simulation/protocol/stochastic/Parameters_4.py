...
class Electrode():
    ...
    def __init__(self,cs_times,cs_duration,cf_a,ss_times,ss_duration,pf_a,background_current):
        self.cs_times = cs_times
        self.cs_duration = cs_duration
        self.cf_a = cf_a

        self.ss_times = ss_times
    ...
    def __call__(self,t):
        ...
        for i,t_cs in enumerate(self.cs_times):
            if t>t_cs and t<t_cs+self.cs_duration:
                current = current+self.cf_a[i]*cf_synapse(t-t_cs)
                break
        for i,t_ss in enumerate(self.ss_times):
            if t>t_ss and t<t_ss+self.ss_duration:
                current = current+self.pf_a[i]*pf_synapse(t-t_ss)
                break
