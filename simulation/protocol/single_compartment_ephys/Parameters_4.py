...
class QuickAndSlow_DoubleExponentialSynapse:

#http://homepages.inf.ed.ac.uk/mvanross/reprints/roth_mvr_chap.pdf

    def __init__(self,w_q,tau_q_rise,tau_q_fall,w_s,tau_s_rise,tau_s_fall):
        
        self.w_q = w_q
        self.tau_q_rise=tau_q_rise
        self.tau_q_fall=tau_q_fall
        self.w_s = w_s
        self.tau_s_rise=tau_s_rise
        self.tau_s_fall=tau_s_fall

        if tau_q_rise>=tau_q_fall:
            print("tau_rise bigger than tau_fall f/p")
        if tau_s_rise>=tau_s_fall:
            print("tau_rise bigger than tau_fall f/p")

        t_q_peak=tau_q_rise*tau_q_fall/(tau_q_fall-tau_q_rise)*log(tau_q_fall/tau_q_rise)
        self.f_q=1/(exp(-t_q_peak/tau_q_fall)-exp(-t_q_peak/tau_q_rise))

        t_s_peak=tau_s_rise*tau_s_fall/(tau_s_fall-tau_s_rise)*log(tau_s_fall/tau_s_rise)
        self.f_s=1/(exp(-t_s_peak/tau_s_fall)-exp(-t_s_peak/tau_s_rise))

    def __call__(self,t):

        #input_current.append(self.f*(exp(-t/self.tau_fall)-exp(-t/self.tau_rise)))
        return self.w_q*self.f_q*(exp(-t/self.tau_q_fall)-exp(-t/self.tau_q_rise))+self.w_s*self.f_s*(exp(-t/self.tau_s_fall)-exp(-t/self.tau_s_rise))

    

# cf_synapse=DoubleExponentialSynapse(tau_rise,tau_decay)
cf_synapse = QuickAndSlow_DoubleExponentialSynapse(w_q,tau_q_rise,tau_q_decay,w_s,tau_s_rise,tau_s_decay)
...
class Electrode():
    ...
    def __init__(self,cs_times,cs_duration,cf_a):
        self.cs_times = cs_times
        self.cs_duration = cs_duration
        self.cf_a = cf_a

    def __call__(self,t):
        for i,t_cs in enumerate(self.cs_times):
            if t>t_cs and t<t_cs+self.cs_duration:
                return background_current+self.cf_a[i]*cf_synapse(t-t_cs)
